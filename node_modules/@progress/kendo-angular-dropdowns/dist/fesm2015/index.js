...
let MultiSelectComponent = MultiSelectComponent_1 = class MultiSelectComponent {
    ...
    removeGroupTag(dataItems) {
        ...
        this.value = this.value.filter(value => !dataItemValues.has(getter$1(value, this.valueField)));
        this.emitValueChange();
    }
...
let MultiSelectTreeComponent = MultiSelectTreeComponent_1 = class MultiSelectTreeComponent {
    constructor(popupService, renderer, hostElement, navigationService, _zone, localization, cdr) {
        this.popupService = popupService;
        this.renderer = renderer;
        this.hostElement = hostElement;
        this.navigationService = navigationService;
     ...
     */
    handleKeydown(event) {
        const deleteTag = !this.isTreeViewActive && event.keyCode === Keys.Backspace && this.tags.length > 0;
        if (deleteTag) {
            this.handleBackspace();
            return;
        }
    ...
    handleRemoveTag({ tag, index }) {
        ...
        if (tag instanceof Array) {
            ...
            this.valueDepth = this.valueDepth.filter((_item, i) => i < this.showAfter);
        }
        else {
            // Remove single tag
            this.dataItems = this.dataItems.filter((_item, i) => i !== index);
            this.valueDepth = this.valueDepth.filter((_item, i) => i !== index);
        }
        this.updateValue(this.dataItems);
...
let FilterDirective = class FilterDirective {
    ...
    normalizeFilterSettings(settings) {
