...
var MultiSelectTreeComponent = /** @class */ (function () {
    function MultiSelectTreeComponent(popupService, renderer, hostElement, navigationService, _zone, localization, cdr) {
        ...
        this.tagListId = kendo_angular_common_1.guid();
        this.tagPrefix = "tag-" + kendo_angular_common_1.guid();
    ...
    Object.defineProperty(MultiSelectTreeComponent.prototype, "tabindex", {
         ...
         */
        set: function (value) {
            var providedTabIndex = util_1.parseNumber(value);
    ...
    Object.defineProperty(MultiSelectTreeComponent.prototype, "dataItems", {
        ...
        set: function (items) {
            ...
            this._dataItems = (items || []).map(function (item, index) {
                if (util_1.hasProps(item, ['dataItem', 'index', 'level'])) {
                    return item;
                }
                return {
                    dataItem: item,
    ...
    Object.defineProperty(MultiSelectTreeComponent.prototype, "focusedTagId", {
         ...
         */
        get: function () {
            if (!util_1.isPresent(this.focusedTagIndex) || this.isOpen) {
                return null;
            }
            var dataItem = this.tags[this.focusedTagIndex];
            return this.tagPrefix + "-" + util_1.valueFrom({ dataItem: dataItem }, this.valueField);
        },
        enumerable: true,
        configurable: true
    });
    ...
    MultiSelectTreeComponent.prototype.ngOnChanges = function (changes) {
        ...
        if (kendo_angular_common_1.anyChanged(['valueDepth', 'value', 'dataItems'], changes, false)) {
    ...
    MultiSelectTreeComponent.prototype.isEmpty = function () {
        return !Boolean(this.placeholder) && (!util_1.isPresent(this.value) || this.value.length === 0);
    };
    /**
    ...
    MultiSelectTreeComponent.prototype.handleKeydown = function (event) {
        var deleteTag = !this.isTreeViewActive && event.keyCode === kendo_angular_common_1.Keys.Backspace && this.tags.length > 0;
        if (deleteTag) {
        ...
        if (action === navigation_action_1.NavigationAction.Open) {
    ...
    MultiSelectTreeComponent.prototype.handleRemoveTag = function (_a) {
        ...
        var eventArgs = new remove_tag_event_1.RemoveTagEvent(tag);
        ...
        if (tag instanceof Array) {
            ...
            this.valueDepth = this.valueDepth.filter(function (_item, i) { return i < _this.showAfter; });
        }
        else {
            // Remove single tag
            this.dataItems = this.dataItems.filter(function (_item, i) { return i !== index; });
            this.valueDepth = this.valueDepth.filter(function (_item, i) { return i !== index; });
        }
        this.updateValue(this.dataItems);
    ...
    MultiSelectTreeComponent.prototype.handleTagMapperChange = function (showAfter) {
        this.showAfter = util_1.parseNumber(showAfter);
        this.setTags();
    };
    /**
     * @hidden
    ...
    MultiSelectTreeComponent.prototype.verifySettings = function () {
        ...
        if (!util_1.isPresent(this.valueField) || !util_1.isPresent(this.textField)) {
            throw new Error(error_messages_1.MultiSelectTreeMessages.textAndValue);
        }
        if (!util_1.isArray(this.value)) {
            throw new Error(error_messages_1.MultiSelectTreeMessages.array);
        }
        if (this.value.length > 0) {
            if (this.valuePrimitive && this.value.some(function (item) { return util_1.isObject(item); })) {
                throw new Error(error_messages_1.MultiSelectTreeMessages.primitive);
            }
            var isEveryDataItemObject = this.dataItems.every(function (item) { return util_1.isObject(item.dataItem); });
            if (this.valuePrimitive && !util_1.isArray(this.dataItems)) {
                throw new Error(error_messages_1.MultiSelectTreeMessages.dataItems);
            }
            if (this.valuePrimitive && !isEveryDataItemObject) {
                throw new Error(error_messages_1.MultiSelectTreeMessages.dataItems);
            }
            if (this.valuePrimitive && this.dataItems.length !== this.value.length) {
                throw new Error(error_messages_1.MultiSelectTreeMessages.dataItemsLength);
            }
            if (!this.valuePrimitive && !util_1.isObjectArray(this.value)) {
                throw new Error(error_messages_1.MultiSelectTreeMessages.object);
            }
            if ((util_1.isArray(this.valueField) || util_1.isArray(this.textField)) && !util_1.isArray(this.valueDepth)) {
                throw new Error(error_messages_1.MultiSelectTreeMessages.valueDepth);
            }
            if ((util_1.isArray(this.valueField) || util_1.isArray(this.textField)) && this.valueDepth.length === 0) {
                throw new Error(error_messages_1.MultiSelectTreeMessages.valueDepth);
            }
            if ((util_1.isArray(this.valueField) || util_1.isArray(this.textField)) && this.valueDepth.length !== this.value.length) {
                throw new Error(error_messages_1.MultiSelectTreeMessages.valueDepthLength);
            }
        }
    };
    ...
    MultiSelectTreeComponent.prototype.subscribeEvents = function () {
        ...
        this.subscriptions.push(this.navigationService.open.subscribe(function () { return _this.togglePopup(true); }), this.navigationService.enter
            ...
            .subscribe(function () { return _this.togglePopup(true); }), rxjs_1.merge(this.navigationService.close, this.navigationService.esc).subscribe(function () {
            _this.focus();
            ...
            .pipe(operators_1.filter(function () { return !_this.isTreeViewActive; }))
            .subscribe(this.direction === 'rtl' ? this.handleRightKey.bind(this) : this.handleLeftKey.bind(this)), this.navigationService.right
            .pipe(operators_1.filter(function () { return !_this.isTreeViewActive; }))
            .subscribe(this.direction === 'rtl' ? this.handleLeftKey.bind(this) : this.handleRightKey.bind(this)), this.navigationService.home.pipe(operators_1.filter(function () { return !_this.isOpen; })).subscribe(this.handleHome.bind(this)), this.navigationService.end.pipe(operators_1.filter(function () { return !_this.isOpen; })).subscribe(this.handleEnd.bind(this)), this.navigationService.backspace.pipe(operators_1.filter(function () { return _this.isTagFocused; })).subscribe(this.handleBackspace.bind(this)), this.navigationService.delete.pipe(operators_1.filter(function () { return _this.isTagFocused; })).subscribe(this.handleDelete.bind(this)));
    };
    ...
    MultiSelectTreeComponent.prototype.handleUpKey = function (event) {
        ...
        if (this.treeview['navigationService']['activeIndex'] === '0') {
            this.focus();
        }
    };
    ...
    MultiSelectTreeComponent.prototype.setState = function () {
        if (util_1.isPresent(this.dataItems) && util_1.isPresent(this.valueField)) {
            this.setTags();
    ...
    MultiSelectTreeComponent.prototype.updateValue = function (value) {
        ...
        var newValue = this.valuePrimitive ?
            value.map(function (item) { return util_1.valueFrom(item, _this.valueField); }) :
        ...
        core_1.Component({
            ...
            providers: [
                ...
                },
                {
                    ...
                    useExisting: core_1.forwardRef(function () { return MultiSelectTreeComponent_1; })
                },
                {
...
exports.MultiSelectTreeComponent = MultiSelectTreeComponent;
